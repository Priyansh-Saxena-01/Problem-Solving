...
int maxi=0;
ds largestBSTsum(Node*root){
    if(root==nullptr) return ds();

    ds leftds=largestBSTsum(root->left);
    ds rightds=largestBSTsum(root->right);
    ...
    ds temp;
    temp.isBst=true;
    temp.sum=leftds.sum+rightds.sum+root->val;
    maxi=max(maxi,temp.sum);
...
int main(){
    ...
    ds ans=largestBSTsum(r);
    cout<<"the largest BST sum is : "<<maxi;
